...
export default function AnimatedAnchor({
  ...
  return (
    <a
      href={href}
      className={`group relative isolate grid overflow-hidden border ${shape === "square" ? "rounded-md" : "rounded-full"} ${inverted ? "border-zinc-900 hover:border-zinc-100" : "border-zinc-100 hover:border-zinc-900"} ${variant.container} transition-colors delay-100 ${className}`}
      {...props}
    >
      <span
        aria-hidden="true"
